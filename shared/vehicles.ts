...
export function calculateQuotePrice(vehicleId: string, distanceKm: number): number | null {
  ...
  // Appliquer la distance minimale
  const effectiveDistance = Math.max(distanceKm, vehicle.minDistance);
  return Math.round(effectiveDistance * vehicle.pricePerKm * 100) / 100;
}
